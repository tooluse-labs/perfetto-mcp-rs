...
- **Chrome trace**：`load_trace` → 直接用专用的 `chrome_*` 工具
  ...
  resource pipeline 钻取单个慢 URL，或用 resource hotspots 做 slice drilldown，
  之后看 script hotspots 判断资源回来后的 JS / style / layout 开销。
  summary 的 `resource_timing_evidence` 会说明是否存在
