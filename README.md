...
| `chrome_page_load_resource_pipeline` | URL drilldown that joins one resource's lifecycle/request spans with background parse, script evaluation, style/layout signals, explicit match evidence, and an evidence boundary for DNS/TLS/TTFB/cache/download hypotheses (Chrome trace) |
